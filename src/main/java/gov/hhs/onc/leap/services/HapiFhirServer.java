package gov.hhs.onc.leap.services;
...
import gov.hhs.onc.leap.services.exceptions.HapiFhirCreateException;
...
public class HapiFhirServer {
    ...
    public void setUp() {
        ...
        clientFactory.setConnectionRequestTimeout(600000);
        clientFactory.setConnectTimeout(600000);
    ...
    public Bundle createAndExecute(Bundle bundle) {
        return hapiClient.transaction()
                .withBundle(bundle)
                .execute();
    }
    ...
    public List<IBaseResource> getAllPatients() {
        ...
        patientList.addAll(BundleUtil.toListOfResources(ctx, bundle));
        while (bundle.getLink(IBaseBundle.LINK_NEXT) != null) {
            bundle = hapiClient
                    .loadPage()
                    .next(bundle)
    ...
    public List<IBaseResource> getAllPatientEncounters(final String patientId) {
        SortSpec sortSpec = new SortSpec();
        sortSpec.setParamName("date");
        sortSpec.setOrder(SortOrderEnum.DESC);
        List<IBaseResource> patientEncounters = new ArrayList<>();
        ...
        patientEncounters.addAll(BundleUtil.toListOfResources(ctx, bundle));
        while (bundle.getLink(IBaseBundle.LINK_NEXT) != null) {
            bundle = hapiClient
                    .loadPage()
                    .next(bundle)
